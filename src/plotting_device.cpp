...
namespace lib {
  ...
  void device(EnvT* e) {
    ...
    // CURSOR_ORIGINAL (WARNING: SAME CODE AS  CURSOR_CROSSHAIR!)
    {
      ...
      BaseGDL* res = e->GetKW(valIx);
      if (res != NULL) {
        bool success = actDevice->CursorCrosshair(true);
        if (!success)
          e->Throw("Current device does not support keyword CURSOR_ORIGINAL.");
      }
    }
    // CURSOR_IMAGE Must be a 16 line by 16 colums bitmap, contained in a 16 short int vector.
    // it comes with CURSOR_MASK and CURSOR_XY
    {
      static int cursorImageIx = e->KeywordIx("CURSOR_IMAGE");
      BaseGDL* what = e->GetKW(cursorImageIx);
      if (what != NULL) {
        DIntGDL* val = e->GetKWAs<DIntGDL>(cursorImageIx);
        if (val->N_Elements() != 16) e->Throw("Keyword array parameter CURSOR_IMAGE must have 16 elements.");
        char* imagegdl = (char*) val->DataAddr();
        char* image=imagegdl;
        if (!BigEndian()) {
          image=(char*)(malloc(64)); //be large
          char k = 0;
          for (int i = 0; i < 31; i += 2) {
            k = imagegdl[i];
            image[i] = imagegdl[i + 1];
            image[i + 1] = k;
          }
        }
        char* maskgdl = NULL;
        char* mask = NULL;
        // it comes with CURSOR_MASK 
        static int cursorMaskIx = e->KeywordIx("CURSOR_MASK");
        if (e->KeywordPresent(cursorMaskIx)) {
          DIntGDL* msk = e->GetKWAs<DIntGDL>(cursorMaskIx);
          if (msk->N_Elements() != 16) e->Throw("Keyword array parameter CURSOR_MASK must have 16 elements.");
          maskgdl = (char*) msk->DataAddr();
          mask=maskgdl;
          if (!BigEndian()) {
            mask=(char*)malloc(64);
            char k = 0;
        ...
        int x = 7;
        int y = 7;
        // it comes with CURSOR_XY 
        static int cursorXYIx = e->KeywordIx("CURSOR_XY");
        if (e->KeywordPresent(cursorXYIx)) {
          DIntGDL* cxy = e->GetKWAs<DIntGDL>(cursorXYIx);
          if (cxy->N_Elements() != 2) e->Throw("Keyword array parameter CURSOR_XY must have 2 elements.");
          x = (*cxy)[0]; x=max(0,min(x,15)); x=15-x; //wx direction
          y = (*cxy)[1]; y=max(0,min(y,15)); 
        }

        bool success = actDevice->CursorImage(image, x, y, mask);
        if (!BigEndian()) {
          free(image);
          if (mask!=NULL) free(mask);
        }
        if (!success)
          e->Throw("Current device does not support keyword CURSOR_IMAGE.");
      }
    }
    // FILENAME 
    {
